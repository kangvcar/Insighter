#!/usr/bin/env bash

echo ""
echo -e "\e[00;36m ▄█  ███▄▄▄▄      ▄████████  ▄█     ▄██████▄     ▄█    █▄        ███        ▄████████    ▄████████      \e[00m";
echo -e "\e[00;36m███  ███▀▀▀██▄   ███    ███ ███    ███    ███   ███    ███   ▀█████████▄   ███    ███   ███    ███      \e[00m";
echo -e "\e[00;36m███▌ ███   ███   ███    █▀  ███▌   ███    █▀    ███    ███      ▀███▀▀██   ███    █▀    ███    ███      \e[00m";
echo -e "\e[00;36m███▌ ███   ███   ███        ███▌  ▄███         ▄███▄▄▄▄███▄▄     ███   ▀  ▄███▄▄▄      ▄███▄▄▄▄██▀      \e[00m";
echo -e "\e[00;36m███▌ ███   ███ ▀███████████ ███▌ ▀▀███ ████▄  ▀▀███▀▀▀▀███▀      ███     ▀▀███▀▀▀     ▀▀███▀▀▀▀▀        \e[00m";
echo -e "\e[00;36m███  ███   ███          ███ ███    ███    ███   ███    ███       ███       ███    █▄  ▀███████████      \e[00m";
echo -e "\e[00;36m███  ███   ███    ▄█    ███ ███    ███    ███   ███    ███       ███       ███    ███   ███    ███      \e[00m";
echo -e "\e[00;36m█▀    ▀█   █▀   ▄████████▀  █▀     ████████▀    ███    █▀       ▄████▀     ██████████   ███    ███      \e[00m";
echo -e "\e[00;36m             Insighter -- 适用于 CentOS & Ubuntu 系统洞察快速检测工具  v1.0             ███    ███           \e[00m";
echo -e "\n"



# WEB Path 设置web目录 默认的话是从/目录去搜索 性能较慢
webpath='/'

### 1.环境检查 ###
prerequisites_setting() {
	echo -e "\e[00;1;33m[+] 环境检测\e[00m"
	# 验证是否为root权限
	if [ $UID -ne 0 ]; then
		echo -e "\n\e[00;5;33m请使用root权限运行 \e[00m"
		exit 1
	else
		echo -e "\e[00;1;32m当前为root权限 \e[00m"
	fi

	# 验证操作系统是debian系还是centos
	OS='None'

	if [ -e "/etc/os-release" ]; then
		source /etc/os-release
		case ${ID} in
			"debian" | "ubuntu" | "devuan")
				OS='Debian'
				;;
			"centos" | "rhel fedora" | "rhel")
				OS='Centos'
				;;
			*) ;;
		esac
	fi

	if [ $OS = 'None' ]; then
		if command -v apt-get >/dev/null 2>&1; then
			OS='Debian'
			if [ -e "silversearcher-ag_2.2.0-1+b1_amd64.deb" ];then
				dpkg -i silversearcher-ag_2.2.0-1+b1_amd64.deb >/dev/null
			fi
		elif command -v yum >/dev/null 2>&1; then
			OS='Centos'
			if [ -e "the_silver_searcher-2.1.0-1.el7.x86_64.rpm" ];then
				rpm -ivh the_silver_searcher-2.1.0-1.el7.x86_64.rpm >/dev/null
			fi
		else
			echo -e "\n\e[41;5;30mInsighter脚本不支持该个系统！已退出\e[00m\n"
			exit 1
		fi
	fi

	# 安装应急必备工具
	cmdline=(
		"epel-release"
		"net-tools"
		"telnet"
		"nc"
		"lrzsz"
		"wget"
		"strace"
		"htop"
		"tar"
		"lsof"
		"tcpdump"
		"the_silver_searcher"
		"silversearcher-ag"
	)
	for prog in "${cmdline[@]}"; do

	if [ $OS = 'Centos' ]; then
		soft=$(rpm -q "$prog")
		if echo "$soft" | grep -E '没有安装|未安装|not installed' >/dev/null 2>&1; then
			echo -e "\033[32m 安装中... \t\t$prog \033[0m"
		yum install -y "$prog" >/dev/null 2>&1
		fi
	else
		if dpkg -L $prog | grep 'does not contain any files' >/dev/null 2>&1; then
			echo -e "\033[32m 安装中... \t\t$prog \033[0m"
			apt install -y "$prog" >/dev/null 2>&1
		fi
	fi
	done
	echo -e "\n"
}

base_check() {
	echo -e "\033[44;1;33m 基础配置检查 \033[0m"

	#系统信息
	echo -e "\e[00;1;33m[+] 系统信息\e[00m"
	#当前用户
	echo -e "USER:\t\t$(whoami)" 2>/dev/null
	#版本信息
	echo -e "OS Version:\t$(uname -r)"
	#主机名
	echo -e "Hostname: \t$(hostname -s)"
	#服务器SN
	echo -e "服务器SN: \t$(dmidecode -t1 | ag -o '(?<=Serial Number: ).*')"
	#uptime
	echo -e "Uptime: \t$(uptime | awk -F ',' '{print $1}')"
	#系统负载
	echo -e "系统负载: \t$(uptime | awk '{print $9" "$10" "$11" "$12" "$13}')"
	#cpu信息
	echo -e "CPU info:\t$(ag -o '(?<=model name\t: ).*' </proc/cpuinfo | head -n 1)"
	#cpu核心
	echo -e "CPU 核心:\t$(cat /proc/cpuinfo | grep 'processor' | sort | uniq | wc -l)"
	#ipaddress
	ipaddress=$(ifconfig | ag -o '(?<=inet |inet addr:)\d+\.\d+\.\d+\.\d+' | ag -v '127.0.0.1') >/dev/null 2>&1
	echo -e "IPADDR:\t\t${ipaddress}" | sed ":a;N;s/\n/ /g;ta"
	echo -e "\n"

	#CPU使用率
	echo -e "\e[00;1;33m[+] CPU使用率  \e[00m"
	echo -e "\e[00;36m[Command]: awk '$0 ~/cpu[0-9]/' /proc/stat 2>/dev/null\e[00m"
	awk '$0 ~/cpu[0-9]/' /proc/stat 2>/dev/null | while read line; do
		echo "$line" | awk '{total=$2+$3+$4+$5+$6+$7+$8;free=$5;\
			print$1" Free "free/total*100"%",\
			"Used " (total-free)/total*100"%"}'
	done
	echo -e "\n"

	#登陆用户
	echo -e "\e[00;1;33m[+] 登陆用户\e[00m"
	echo -e "\e[00;36m[Command]: who\e[00m"
	who
	echo -e "\n"

	#CPU占用TOP 10
	cpu=$(ps aux | grep -v ^'USER' | sort -rn -k3 | head -10) 2>/dev/null
	echo -e "\e[00;1;33m[+] CPU TOP10  \e[00m"
	echo -e "\e[00;36m[Command]: ps aux | grep -v ^'USER' | sort -rn -k3 | head -10 2>/dev/null\e[00m"
	echo -e "USER        PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND"
	echo -e "${cpu}"
	echo -e "\n"

	#内存占用TOP 10
	mem=$(ps aux | grep -v ^'USER' | sort -rn -k4 | head -10) 2>/dev/null
	echo -e "\e[00;1;33m[+] 内存占用 TOP10  \e[00m"
	echo -e "\e[00;36m[Command]: ps aux | grep -v ^'USER' | sort -rn -k4 | head -10 2>/dev/null\e[00m"
	echo -e "USER        PID %CPU %MEM    VSZ   RSS TTY      STAT START   TIME COMMAND"
	echo -e "${mem}"

	#内存占用
	echo -e "\e[00;1;33m[+] 内存占用\e[00m"
	echo -e "\e[00;36m[Command]: free -mh\e[00m"
	free -mh
	echo -e "\n"

	#剩余空间
	echo -e "\e[00;1;33m[+] 剩余空间\e[00m"
	echo -e "\e[00;36m[Command]: df -mh\e[00m"
	df -mh
	echo -e "\n"

	#硬盘挂载
	echo -e "\e[00;1;33m[+] 硬盘挂载\e[00m"
	echo -e "\e[00;36m[Command]: ag -v '#' </etc/fstab | awk '{print \$1,\$2,\$3}'\e[00m"
	ag -v "#" </etc/fstab | awk '{print $1,$2,$3}'
	echo -e "\n"

	#安装软件
	echo -e "\e[00;1;33m[+] 常用软件\e[00m"
	echo -e "\e[00;36m[Command]: which (perl|gcc|g++|python|php|cc|go|node|nodejs|bind|tomcat|clang|ruby|curl|wget|mysql|redis|ssserver|vsftpd|java|apache|apache2|nginx|git|mongodb|docker|tftp|psql|kafka)\e[00m"
	cmdline=(
		"which perl"
		"which gcc"
		"which g++"
		"which python"
		"which php"
		"which cc"
		"which go"
		"which node"
		"which nodejs"
		"which bind"
		"which tomcat"
		"which clang"
		"which ruby"
		"which curl"
		"which wget"
		"which mysql"
		"which redis"
		"which ssserver"
		"which vsftpd"
		"which java"
		"which apache"
		"which apache2"
		"which nginx"
		"which git"
		"which mongodb"
		"which docker"
		"which tftp"
		"which psql"
		"which kafka"
	)

	for prog in "${cmdline[@]}"; do
		soft=$($prog)
		if [ "$soft" ] 2>/dev/null; then
			echo -e "$soft" | ag -o '\w+$' --nocolor
		fi
	done
	echo -e "\n"

	#HOSTS
	echo -e "\e[00;1;33m[+] /etc/hosts \e[00m"
	echo -e "\e[00;36m[Command]: cat /etc/hosts | ag -v '#'\e[00m"
	cat /etc/hosts | ag -v "#"
	echo -e "\n"
}

network_check() {
	echo -e "\033[44;1;33m 网络&流量检查 \033[0m"

	#ifconfig
	echo -e "\e[00;1;33m[+] ifconfig\e[00m"
	echo -e "\e[00;36m[Command]: /sbin/ifconfig -a\e[00m"
	/sbin/ifconfig -a
	echo -e "\n"

	#网络流量
	echo -e "\e[00;1;33m[+] 网络流量 \e[00m"
	echo -e "\e[00;36m[Command]: awk ' NR>2' /proc/net/dev\e[00m"
	echo "Interface    ByteRec   PackRec   ByteTran   PackTran"
	awk ' NR>2' /proc/net/dev | while read line; do
		echo "$line" | awk -F ':' '{print "  "$1"  " $2}' |
		awk '{print $1"   "$2 "    "$3"   "$10"  "$11}'
	done
	echo -e "\n"

	#端口监听
	echo -e "\e[00;1;33m[+] 端口监听\e[00m"
	echo -e "\e[00;36m[Command]: netstat -tulpen | ag 'tcp|udp.*'\e[00m"
	echo "Proto Recv-Q Send-Q Local Address           Foreign Address         State       User       Inode      PID/Program name"
	netstat -tulpen | ag 'tcp|udp.*' --nocolor
	echo -e "\n"

	#对外开放端口
	echo -e "\e[00;1;33m[+] 对外开放端口\e[00m"
	echo -e "\e[00;36m[Command]: netstat -tulpen | awk '{print $1,$4}' | ag -o '.*0.0.0.0:(\d+)|:::\d+'\e[00m"
	echo "Proto Local Address"
	netstat -tulpen | awk '{print $1,$4}' | ag -o '.*0.0.0.0:(\d+)|:::\d+' --nocolor
	echo -e "\n"

	#网络连接
	echo -e "\e[00;1;33m[+] 已建立的网络连接\e[00m"
	echo -e "\e[00;36m[Command]: netstat -antop | ag ESTAB\e[00m"
	echo -e "Proto Recv-Q Send-Q Local Address           Foreign Address         State       PID/Program name     Timer"
	netstat -antop | ag ESTAB --nocolor
	echo -e "\n"

	#连接状态
	echo -e "\e[00;1;33m[+] TCP连接状态\e[00m"
	echo -e "\e[00;36m[Command]: netstat -n | awk '/^tcp/ {++S[$NF]} END {for(a in S) print a, S[a]}'\e[00m"
	echo -e "State\tCount"
	netstat -n | awk '/^tcp/ {++S[$NF]} END {for(a in S) print a, S[a]}'
	echo -e "\n"

	#路由表
	echo -e "\e[00;1;33m[+] 路由表\e[00m"
	echo -e "\e[00;36m[Command]: /sbin/route -nee\e[00m"
	/sbin/route -nee
	echo -e "\n"

	#路由转发
	echo -e "\e[00;1;33m[+] 路由转发\e[00m"
	echo -e "\e[00;36m[Command]: more /proc/sys/net/ipv4/ip_forward | awk -F: '{if ($1==1) print "1"}'\e[00m"
	ip_forward=$(more /proc/sys/net/ipv4/ip_forward | awk -F: '{if ($1==1) print "1"}')
	if [ -n "$ip_forward" ]; then
		echo "/proc/sys/net/ipv4/ip_forward 已开启路由转发"
	else
		echo "该服务器未开启路由转发"
	fi
	echo -e "\n"

	#DNS
	echo -e "\e[00;1;33m[+] DNS Server\e[00m"
	echo -e "\e[00;36m[Command]: ag -o '\d+\.\d+\.\d+\.\d+' --nocolor </etc/resolv.conf\e[00m"
	ag -o '\d+\.\d+\.\d+\.\d+' --nocolor </etc/resolv.conf
	echo -e "\n"

	#ARP
	echo -e "\e[00;1;33m[+] ARP\e[00m"
	echo -e "\e[00;36m[Command]: arp -n -a\e[00m"
	arp -n -a
	echo -e "\n"

	#混杂模式
	echo -e "\e[00;1;33m[+] 网卡混杂模式\e[00m"
	echo -e "\e[00;36m[Command]: ip link | ag PROMISC\e[00m"
	if ip link | ag PROMISC >/dev/null 2>&1; then
		echo "网卡存在混杂模式！"
	else
		echo "网卡不存在混杂模式"
	fi
	echo -e "\n"

	#防火墙
	echo -e "\e[00;1;33m[+] IPTABLES防火墙\e[00m"
	echo -e "\e[00;36m[Command]: iptables -L\e[00m"
	iptables -L
	echo -e "\n"
}

crontab_check() {
	echo -e "\033[44;1;33m 任务计划检查 \033[0m"
	#crontab
	echo -e "\e[00;1;33m[+] Crontab\e[00m"
	echo -e "\e[00;36m[Command]: crontab -u root -l | ag -v '#'\e[00m"
	crontab -u root -l | ag -v '#' --nocolor
	echo -e "\n"

	echo -e "\e[00;36m[Command]: ls -alht /etc/cron.*/*\e[00m"
	ls -alht /etc/cron.*/*
	echo -e "\n"

	#crontab可疑命令
	echo -e "\e[00;1;33m[+] Crontab Backdoor \e[00m"
	echo -e "\e[00;36m[Command]: ag '((?:useradd|groupadd|chattr)|(?:wget\s|curl\s|tftp\s\-i|scp\s|sftp\s)|(?:bash\s\-i|fsockopen|nc\s\-e|sh\s\-i|\"/bin/sh\"|\"/bin/bash\"))' /etc/cron* /var/spool/cron/*\e[00m"
	ag '((?:useradd|groupadd|chattr)|(?:wget\s|curl\s|tftp\s\-i|scp\s|sftp\s)|(?:bash\s\-i|fsockopen|nc\s\-e|sh\s\-i|\"/bin/sh\"|\"/bin/bash\"))' /etc/cron* /var/spool/cron/* --nocolor
	echo -e "\n"
}

env_check() {
	echo -e "\033[44;1;33m 环境变量检查 \033[0m"
	#env
	echo -e "\e[00;1;33m[+] env\e[00m"
	echo -e "\e[00;36m[Command]: env\e[00m"
	env
	echo -e "\n"

	#PATH
	echo -e "\e[00;1;33m[+] PATH\e[00m"
	echo -e "\e[00;36m[Command]: echo \$PATH\e[00m"
	echo "$PATH"
	echo -e "\n"

	#LD_PRELOAD
	echo -e "\e[00;1;33m[+] LD_PRELOAD\e[00m"
	echo -e "\e[00;36m[Command]: echo \${LD_PRELOAD}\e[00m"
	if [ ${LD_PRELOAD} ];then
		echo ${LD_PRELOAD}
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"
	
	#LD_ELF_PRELOAD
	echo -e "\e[00;1;33m[+] LD_ELF_PRELOAD\e[00m"
	echo -e "\e[00;36m[Command]: echo \${LD_ELF_PRELOAD}\e[00m"
	if [ ${LD_ELF_PRELOAD} ];then
		echo ${LD_ELF_PRELOAD}
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"

	#LD_AOUT_PRELOAD
	echo -e "\e[00;1;33m[+] LD_AOUT_PRELOAD\e[00m"
	echo -e "\e[00;36m[Command]: echo \${LD_AOUT_PRELOAD}\e[00m"
	if [ ${LD_AOUT_PRELOAD} ];then
		echo ${LD_AOUT_PRELOAD}
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"

	#PROMPT_COMMAND
	echo -e "\e[00;1;33m[+] PROMPT_COMMAND\e[00m"
	echo -e "\e[00;36m[Command]: echo \${PROMPT_COMMAND}\e[00m"
	if [ ${PROMPT_COMMAND} ];then
		echo ${PROMPT_COMMAND}
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"

	#LD_LIBRARY_PATH
	echo -e "\e[00;1;33m[+] LD_LIBRARY_PATH\e[00m"
	echo -e "\e[00;36m[Command]: echo \${LD_LIBRARY_PATH}\e[00m"
	if [ ${LD_LIBRARY_PATH} ];then
		echo ${LD_LIBRARY_PATH}
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"

	#ld.so.preload
	echo -e "\e[00;1;33m[+] ld.so.preload\e[00m"
	echo -e "\e[00;36m[Command]: cat \${preload}\e[00m"
	preload='/etc/ld.so.preload'
	if [ -e "${preload}" ]; then
		cat ${preload}
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"
}

user_check() {
	echo -e "\033[44;1;33m 用户信息检查 \033[0m"
	echo -e "\e[00;1;33m[+] 可登陆的用户\e[00m"
	echo -e "\e[00;36m[Command]: cat /etc/passwd | ag -v 'nologin$|false$'\e[00m"
	cat /etc/passwd | ag -v 'nologin$|false$'
	echo -e "\n"
	echo -e "\e[00;1;33m[+] passwd文件修改日期\e[00m"
	echo -e "\e[00;36m[Command]: stat /etc/passwd | ag -o '(?<=Modify:|最近更改：).*'\e[00m"
	stat /etc/passwd | ag -o '(?<=Modify:|最近更改：).*' --nocolor
	echo -e "\n"
	echo -e "\e[00;1;33m[+] sudoers(请注意NOPASSWD)\e[00m"
	echo -e "\e[00;36m[Command]: cat /etc/sudoers | ag -v '#' | sed -e '/^$/d' | ag ALL\e[00m"
	cat /etc/sudoers | ag -v '#' | sed -e '/^$/d' | ag ALL --nocolor
	echo -e "\n"
	echo -e "\e[00;1;33m[+] 登录信息\e[00m"
	echo -e "\e[00;36m[Command]: w\e[00m"
	w
	echo -e "\n"
	echo -e "\e[00;36m[Command]: last\e[00m"
	last
	echo -e "\n"
	echo -e "\e[00;36m[Command]: lastlog\e[00m"
	lastlog
	echo -e "\n"
	echo -e "\e[00;1;33m[+] 登陆成功的IP\e[00m"
	echo -e "\e[00;36m[Command]: ag -a accepted /var/log/secure /var/log/auth.* 2>/dev/null | ag -o '\d+\.\d+\.\d+\.\d+' | sort | uniq\e[00m"
	echo "$(ag -a accepted /var/log/secure /var/log/auth.* 2>/dev/null | ag -o '\d+\.\d+\.\d+\.\d+' | sort | uniq)"
	echo -e "\n"
}

service_check() {
	echo -e "\033[44;1;33m 服务状态检查 \033[0m"
	echo -e "\e[00;1;33m[+] 正在运行的Service \e[00m"
	echo -e "\e[00;36m[Command]: systemctl -l | grep running | awk '{print \$1}'\e[00m"
	systemctl -l | grep running | awk '{print $1}'
	echo -e "\n"
	echo -e "\e[00;1;33m[+] 最近添加的Service \e[00m"
	echo -e "\e[00;36m[Command]: ls -alhtR /etc/systemd/system/multi-user.target.wants\e[00m"
	ls -alhtR /etc/systemd/system/multi-user.target.wants
	echo -e "\n"
	echo -e "\e[00;36m[Command]: ls -alht /etc/systemd/system/*.wants/*.service | ag -v 'dbus-org'\e[00m"
	ls -alht /etc/systemd/system/*.wants/*.service | ag -v 'dbus-org'
	echo -e "\n"
}

bash_check() {
	echo -e "\033[44;1;33m Bash配置检查 \033[0m"
	#查看history文件
	echo -e "\e[00;1;33m[+] History\e[00m"
	echo -e "\e[00;36m[Command]: ls -alht /root/.*_history\e[00m"
	ls -alht /root/.*_history
	echo -e "\n"

	echo -e "\e[00;36m[Command]: cat ~/.*history | ag '(?<![0-9])(?:(?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2})[.](?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2})[.](?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2})[.](?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2}))(?![0-9])|http://|https://|\bssh\b|\bscp\b|\.tar|\bwget\b|\bcurl\b|\bnc\b|\btelnet\b|\bbash\b|\bsh\b|\bchmod\b|\bchown\b|/etc/passwd|/etc/shadow|/etc/hosts|\bnmap\b|\bfrp\b|\bnfs\b|\bsshd\b|\bmodprobe\b|\blsmod\b|\bsudo\b' --nocolor | ag -v 'man\b|ag\b|cat\b|sed\b|git\b|docker\b|rm\b|touch\b|mv\b|\bapt\b|\bapt-get\b'\e[00m"
	cat ~/.*history | ag '(?<![0-9])(?:(?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2})[.](?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2})[.](?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2})[.](?:25[0-5]|2[0-4][0-9]|[0-1]?[0-9]{1,2}))(?![0-9])|http://|https://|\bssh\b|\bscp\b|\.tar|\bwget\b|\bcurl\b|\bnc\b|\btelnet\b|\bbash\b|\bsh\b|\bchmod\b|\bchown\b|/etc/passwd|/etc/shadow|/etc/hosts|\bnmap\b|\bfrp\b|\bnfs\b|\bsshd\b|\bmodprobe\b|\blsmod\b|\bsudo\b' --nocolor | ag -v 'man\b|ag\b|cat\b|sed\b|git\b|docker\b|rm\b|touch\b|mv\b|\bapt\b|\bapt-get\b'
	echo -e "\n"

	#/etc/profile
	echo -e "\e[00;1;33m[+] /etc/profile \e[00m"
	echo -e "\e[00;36m[Command]: cat /etc/profile | ag -v '#'\e[00m"
	cat /etc/profile | ag -v '#'
	echo -e "\n"

	# $HOME/.profile
	echo -e "\e[00;1;33m[+] \$HOME/.profile \e[00m"
	echo -e "\e[00;36m[Command]: cat $HOME/.profile | ag -v '#'\e[00m"
	cat $HOME/.profile | ag -v '#'
	echo -e "\n"

	#/etc/rc.local
	echo -e "\e[00;1;33m[+] /etc/rc.local \e[00m"
	echo -e "\e[00;36m[Command]: cat /etc/rc.local | ag -v '#'\e[00m"
	cat /etc/rc.local | ag -v '#'
	echo -e "\n"

	#~/.bash_profile
	echo -e "\e[00;1;33m[+] ~/.bash_profile \e[00m"
	echo -e "\e[00;36m[Command]: cat ~/.bash_profile | ag -v '#'\e[00m"
	if [ -e "$HOME/.bash_profile" ]; then
		cat ~/.bash_profile | ag -v '#'
	fi
	echo -e "\n"

	#~/.bashrc
	echo -e "\e[00;1;33m[+] ~/.bashrc \e[00m"
	echo -e "\e[00;36m[Command]: cat ~/.bashrc | ag -v '#'\e[00m"
	cat ~/.bashrc | ag -v '#'
	echo -e "\n"

	#bash反弹shell
	echo -e "\e[00;1;33m[+] bash反弹shell \e[00m"
	echo -e "\e[00;36m[Command]: ps -ef | ag 'bash -i' | ag -v 'ag' | awk '{print \$2}' | xargs -i{} lsof -p {} | ag 'ESTAB'\e[00m"
	bash_reverse_shell=$(ps -ef | ag 'bash -i' | ag -v 'ag' | awk '{print $2}' | xargs -i{} lsof -p {} | ag 'ESTAB' --nocolor)
	if [ $bash_reverse_shell ];then
		echo $bash_reverse_shell
	else
		echo -e "\e[00;1;32m[PASS]\e[00m"
	fi
	echo -e "\n"
}

file_check() {
	echo -e "\033[44;1;33m 文件检查 \033[0m"
	#系统文件修改时间
	echo -e "\e[00;1;33m[+] 系统文件修改时间 \e[00m"
	echo -e "\e[00;36m[Command]: stat (/sbin/ifconfig|/bin/ls|/bin/login|/bin/netstat|/bin/top|/bin/ps|/bin/find|/bin/grep|/etc/passwd|/etc/shadow|/usr/bin/curl|/usr/bin/wget|/root/.ssh/authorized_keys)\e[00m"
	cmdline=(
		"/sbin/ifconfig"
		"/bin/ls"
		"/bin/login"
		"/bin/netstat"
		"/bin/top"
		"/bin/ps"
		"/bin/find"
		"/bin/grep"
		"/etc/passwd"
		"/etc/shadow"
		"/usr/bin/curl"
		"/usr/bin/wget"
		"/root/.ssh/authorized_keys"
	)
	for soft in "${cmdline[@]}"; do
		if [ `command -v $soft` ];then
			echo -e "文件：$soft\t\t\t修改日期：$(stat $soft | ag -o '(?<=Modify:|最近更改：)[\d-\s:]+')"
		fi
	done
	echo -e "\n"

	#隐藏文件
	echo -e "\e[00;1;33m[+] ...隐藏文件 \e[00m"
	echo -e "\e[00;36m[Command]: find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -name ".*."\e[00m"
	find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -name ".*."
	echo -e "\n"

	#tmp目录
	echo -e "\e[00;1;33m[+] /tmp \e[00m"
	echo -e "\e[00;36m[Command]: ls -alht /tmp /var/tmp /dev/shm \e[00m"
	ls -alht /tmp /var/tmp /dev/shm 
	echo -e "\n"

	#alias 别名
	echo -e "\e[00;1;33m[+] alias \e[00m"
	echo -e "\e[00;36m[Command]: alias | ag -v 'git' \e[00m"
	alias | ag -v 'git'
	echo -e "\n"

	#SUID
	echo -e "\e[00;1;33m[+] SUID \e[00m"
	echo -e "\e[00;36m[Command]: find / ! -path "/proc/*" -perm -004000 -type f | ag -v 'snap|docker|pam_timestamp_check|unix_chkpwd|ping|mount|su|pt_chown|ssh-keysign|at|passwd|chsh|crontab|chfn|usernetctl|staprun|newgrp|chage|dhcp|helper|pkexec|top|Xorg|nvidia-modprobe|quota|login|security_authtrampoline|authopen|traceroute6|traceroute|ps'\e[00m"
	find / ! -path "/proc/*" -perm -004000 -type f | ag -v 'snap|docker|pam_timestamp_check|unix_chkpwd|ping|mount|su|pt_chown|ssh-keysign|at|passwd|chsh|crontab|chfn|usernetctl|staprun|newgrp|chage|dhcp|helper|pkexec|top|Xorg|nvidia-modprobe|quota|login|security_authtrampoline|authopen|traceroute6|traceroute|ps'
	echo -e "\n"

	#进程存在但文件已经没有了
	echo -e "\e[00;1;33m[+] lsof +L1 \e[00m"
	echo -e "\e[00;36m[Command]: lsof +L1\e[00m"
	lsof +L1
	echo -e "\n"

	#近7天改动
	echo -e "\e[00;1;33m[+] 近七天文件改动 mtime \e[00m"
	echo -e "\e[00;36m[Command]: find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -mtime -7 -type f | ag -v '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n|vscode' | xargs -i{} ls -alh {}\e[00m"
	find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -mtime -7 -type f | ag -v '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n|vscode' | xargs -i{} ls -alh {}
	echo -e "\n"

	#近7天改动
	echo -e "\e[00;1;33m[+] 近七天文件改动 ctime \e[00m"
	echo -e "\e[00;36m[Command]: find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -ctime -7 -type f | ag -v '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n|vscode|git-core|perl5' | xargs -i{} ls -alh {}\e[00m"
	find /etc /bin /lib /sbin /dev /root/ /home /tmp /var /usr ! -path "/var/log*" ! -path "/var/spool/exim4*" ! -path "/var/backups*" -ctime -7 -type f | ag -v '\.log|cache|vim|/share/|/lib/|.zsh|.gem|\.git|LICENSE|README|/_\w+\.\w+|\blogs\b|elasticsearch|nohup|i18n|vscode|git-core|perl5' | xargs -i{} ls -alh {}
	echo -e "\n"

	#大文件200mb
	#黑客可能会将数据库、网站打包成一个文件然后下载
	echo -e "\e[00;1;33m[+] 大文件>200mb \e[00m"
	echo -e "\e[00;36m[Command]: find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -size +200M -exec ls -alht {} + 2>/dev/null | ag '\.gif|\.jpeg|\.jpg|\.png|\.zip|\.tar.gz|\.tgz|\.7z|\.log|\.xz|\.rar|\.bak|\.old|\.sql|\.1|\.txt|\.tar|\.db|/\w+$' --nocolor | ag -v 'ib_logfile|ibd|mysql-bin|mysql-slow|ibdata1'\e[00m"
	find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -size +200M -exec ls -alht {} + 2>/dev/null | ag '\.gif|\.jpeg|\.jpg|\.png|\.zip|\.tar.gz|\.tgz|\.7z|\.log|\.xz|\.rar|\.bak|\.old|\.sql|\.1|\.txt|\.tar|\.db|/\w+$' --nocolor | ag -v 'ib_logfile|ibd|mysql-bin|mysql-slow|ibdata1'
	echo -e "\n"

	#敏感文件
	echo -e "\e[00;1;33m[+] 敏感文件 \e[00m"
	echo -e "\e[00;36m[Command]: find / ! -path "/lib/modules*" ! -path "/usr/src*" ! -path "/snap*" ! -path "/usr/include/*" -regextype posix-extended -regex '.*sqlmap|.*msfconsole|.*\bncat|.*\bnmap|.*nikto|.*ettercap|.*tunnel\.(php|jsp|asp|py)|.*/nc\b|.*socks.(php|jsp|asp|py)|.*proxy.(php|jsp|asp|py)|.*brook.*|.*frps|.*frpc|.*aircrack|.*hydra|.*miner|.*/ew$' -type f | ag -v '/lib/python' | xargs -i{} ls -alh {}\e[00m"
	find / ! -path "/lib/modules*" ! -path "/usr/src*" ! -path "/snap*" ! -path "/usr/include/*" -regextype posix-extended -regex '.*sqlmap|.*msfconsole|.*\bncat|.*\bnmap|.*nikto|.*ettercap|.*tunnel\.(php|jsp|asp|py)|.*/nc\b|.*socks.(php|jsp|asp|py)|.*proxy.(php|jsp|asp|py)|.*brook.*|.*frps|.*frpc|.*aircrack|.*hydra|.*miner|.*/ew$' -type f | ag -v '/lib/python' | xargs -i{} ls -alh {}
	echo -e "\n"

	echo -e "\e[00;1;33m[+] 可疑黑客文件 \e[00m"
	echo -e "\e[00;36m[Command]: find /root /home /opt /tmp /var/ /dev -regextype posix-extended -regex '.*wget|.*curl|.*openssl|.*mysql' -type f 2>/dev/null | xargs -i{} ls -alh {} | ag -v '/pkgs/|/envs/'\e[00m"
	find /root /home /opt /tmp /var/ /dev -regextype posix-extended -regex '.*wget|.*curl|.*openssl|.*mysql' -type f 2>/dev/null | xargs -i{} ls -alh {} | ag -v '/pkgs/|/envs/'
	echo -e "\n"
}

rootkit_check() {
	echo -e "\033[44;1;33m Rootkit检查 \033[0m"
	#lsmod 可疑模块
	echo -e "\e[00;1;33m[+] lsmod 可疑模块\e[00m"
	echo -e "\e[00;36m[Command]: lsmod | ag -v 'ablk_helper|ac97_bus|acpi_power_meter|aesni_intel|ahci|ata_generic|ata_piix|auth_rpcgss|binfmt_misc|bluetooth|bnep|bnx2|bridge|cdrom|cirrus|coretemp|crc_t10dif|crc32_pclmul|crc32c_intel|crct10dif_common|crct10dif_generic|crct10dif_pclmul|cryptd|dca|dcdbas|dm_log|dm_mirror|dm_mod|dm_region_hash|drm|drm_kms_helper|drm_panel_orientation_quirks|e1000|ebtable_broute|ebtable_filter|ebtable_nat|ebtables|edac_core|ext4|fb_sys_fops|floppy|fuse|gf128mul|ghash_clmulni_intel|glue_helper|grace|i2c_algo_bit|i2c_core|i2c_piix4|i7core_edac|intel_powerclamp|ioatdma|ip_set|ip_tables|ip6_tables|ip6t_REJECT|ip6t_rpfilter|ip6table_filter|ip6table_mangle|ip6table_nat|ip6ta ble_raw|ip6table_security|ipmi_devintf|ipmi_msghandler|ipmi_si|ipmi_ssif|ipt_MASQUERADE|ipt_REJECT|iptable_filter|iptable_mangle|iptable_nat|iptable_raw|iptable_security|iTCO_vendor_support|iTCO_wdt|jbd2|joydev|kvm|kvm_intel|libahci|libata|libcrc32c|llc|lockd|lpc_ich|lrw|mbcache|megaraid_sas|mfd_core|mgag200|Module|mptbase|mptscsih|mptspi|nf_conntrack|nf_conntrack_ipv4|nf_conntrack_ipv6|nf_defrag_ipv4|nf_defrag_ipv6|nf_nat|nf_nat_ipv4|nf_nat_ipv6|nf_nat_masquerade_ipv4|nfnetlink|nfnetlink_log|nfnetlink_queue|nfs_acl|nfsd|parport|parport_pc|pata_acpi|pcspkr|ppdev|rfkill|sch_fq_codel|scsi_transport_spi|sd_mod|serio_raw|sg|shpchp|snd|snd_ac97_codec|snd_ens1371|snd_page_alloc|snd_pcm|snd_rawmidi|snd_seq|snd_seq_device|snd_seq_midi|snd_seq_midi_event|snd_timer|soundcore|sr_mod|stp|sunrpc|syscopyarea|sysfillrect|sysimgblt|tcp_lp|ttm|tun|uvcvideo|videobuf2_core|videobuf2_memops|videobuf2_vmalloc|videodev|virtio|virtio_balloon|virtio_console|virtio_net|virtio_pci|virtio_ring|virtio_scsi|vmhgfs|vmw_balloon|vmw_vmci|vmw_vsock_vmci_transport|vmware_balloon|vmwgfx|vsock|xfs|xt_CHECKSUM|xt_conntrack|xt_state|raid*|tcpbbr|btrfs|.*diag|psmouse|ufs|linear|msdos|cpuid|veth|xt_tcpudp|xfrm_user|xfrm_algo|xt_addrtype|br_netfilter|input_leds|sch_fq|ib_iser|rdma_cm|iw_cm|ib_cm|ib_core|.*scsi.*|tcp_bbr|pcbc|autofs4|multipath|hfs.*|minix|ntfs|vfat|jfs|usbcore|usb_common|ehci_hcd|uhci_hcd|ecb|crc32c_generic|button|hid|usbhid|evdev|hid_generic|overlay|xt_nat|qnx4|sb_edac|acpi_cpufreq|ixgbe|pf_ring|tcp_htcp|cfg80211|x86_pkg_temp_thermal|mei_me|mei|processor|thermal_sys|lp|enclosure|ses|ehci_pci|igb|i2c_i801|pps_core|isofs|nls_utf8|xt_REDIRECT|xt_multiport|iosf_mbi|qxl|cdc_ether|usbnet'\e[00m"
	lsmod | ag -v "ablk_helper|ac97_bus|acpi_power_meter|aesni_intel|ahci|ata_generic|ata_piix|auth_rpcgss|binfmt_misc|bluetooth|bnep|bnx2|bridge|cdrom|cirrus|coretemp|crc_t10dif|crc32_pclmul|crc32c_intel|crct10dif_common|crct10dif_generic|crct10dif_pclmul|cryptd|dca|dcdbas|dm_log|dm_mirror|dm_mod|dm_region_hash|drm|drm_kms_helper|drm_panel_orientation_quirks|e1000|ebtable_broute|ebtable_filter|ebtable_nat|ebtables|edac_core|ext4|fb_sys_fops|floppy|fuse|gf128mul|ghash_clmulni_intel|glue_helper|grace|i2c_algo_bit|i2c_core|i2c_piix4|i7core_edac|intel_powerclamp|ioatdma|ip_set|ip_tables|ip6_tables|ip6t_REJECT|ip6t_rpfilter|ip6table_filter|ip6table_mangle|ip6table_nat|ip6ta ble_raw|ip6table_security|ipmi_devintf|ipmi_msghandler|ipmi_si|ipmi_ssif|ipt_MASQUERADE|ipt_REJECT|iptable_filter|iptable_mangle|iptable_nat|iptable_raw|iptable_security|iTCO_vendor_support|iTCO_wdt|jbd2|joydev|kvm|kvm_intel|libahci|libata|libcrc32c|llc|lockd|lpc_ich|lrw|mbcache|megaraid_sas|mfd_core|mgag200|Module|mptbase|mptscsih|mptspi|nf_conntrack|nf_conntrack_ipv4|nf_conntrack_ipv6|nf_defrag_ipv4|nf_defrag_ipv6|nf_nat|nf_nat_ipv4|nf_nat_ipv6|nf_nat_masquerade_ipv4|nfnetlink|nfnetlink_log|nfnetlink_queue|nfs_acl|nfsd|parport|parport_pc|pata_acpi|pcspkr|ppdev|rfkill|sch_fq_codel|scsi_transport_spi|sd_mod|serio_raw|sg|shpchp|snd|snd_ac97_codec|snd_ens1371|snd_page_alloc|snd_pcm|snd_rawmidi|snd_seq|snd_seq_device|snd_seq_midi|snd_seq_midi_event|snd_timer|soundcore|sr_mod|stp|sunrpc|syscopyarea|sysfillrect|sysimgblt|tcp_lp|ttm|tun|uvcvideo|videobuf2_core|videobuf2_memops|videobuf2_vmalloc|videodev|virtio|virtio_balloon|virtio_console|virtio_net|virtio_pci|virtio_ring|virtio_scsi|vmhgfs|vmw_balloon|vmw_vmci|vmw_vsock_vmci_transport|vmware_balloon|vmwgfx|vsock|xfs|xt_CHECKSUM|xt_conntrack|xt_state|raid*|tcpbbr|btrfs|.*diag|psmouse|ufs|linear|msdos|cpuid|veth|xt_tcpudp|xfrm_user|xfrm_algo|xt_addrtype|br_netfilter|input_leds|sch_fq|ib_iser|rdma_cm|iw_cm|ib_cm|ib_core|.*scsi.*|tcp_bbr|pcbc|autofs4|multipath|hfs.*|minix|ntfs|vfat|jfs|usbcore|usb_common|ehci_hcd|uhci_hcd|ecb|crc32c_generic|button|hid|usbhid|evdev|hid_generic|overlay|xt_nat|qnx4|sb_edac|acpi_cpufreq|ixgbe|pf_ring|tcp_htcp|cfg80211|x86_pkg_temp_thermal|mei_me|mei|processor|thermal_sys|lp|enclosure|ses|ehci_pci|igb|i2c_i801|pps_core|isofs|nls_utf8|xt_REDIRECT|xt_multiport|iosf_mbi|qxl|cdc_ether|usbnet"
	echo -e "\n"
	
	#Rootkit 内核模块
	echo -e "\e[00;1;33m[+] Rootkit 内核模块\e[00m"
	echo -e "\e[00;36m[Command]: grep -E 'hide_tcp4_port|hidden_files|hide_tcp6_port|diamorphine|module_hide|module_hidden|is_invisible|hacked_getdents|hacked_kill|heroin|kernel_unlink|hide_module|find_sys_call_tbl|h4x_delete_module|h4x_getdents64|h4x_kill|h4x_tcp4_seq_show|new_getdents|old_getdents|should_hide_file_name|should_hide_task_name' </proc/kallsyms\e[00m"
	kernel=$(grep -E 'hide_tcp4_port|hidden_files|hide_tcp6_port|diamorphine|module_hide|module_hidden|is_invisible|hacked_getdents|hacked_kill|heroin|kernel_unlink|hide_module|find_sys_call_tbl|h4x_delete_module|h4x_getdents64|h4x_kill|h4x_tcp4_seq_show|new_getdents|old_getdents|should_hide_file_name|should_hide_task_name' </proc/kallsyms)
	if [ -n "$kernel" ]; then
		echo -e "\e[41;5;30m 存在内核敏感函数！疑似Rootkit内核模块\e[00m"
		echo "$kernel"
	else
		echo -e "\e[00;1;32m[PASS] 未找到敏感Rootkit 内核模块\e[00m"
	fi
	echo -e "\n"

	#可疑的.ko模块
	echo -e "\e[00;1;33m[+] 可疑的.ko模块\e[00m"
	echo -e "\e[00;36m[Command]: find / ! -path "/proc/*" ! -path "/usr/lib/modules/*" ! -path "/boot/*" -regextype posix-extended -regex '.*\.ko'\e[00m"
	ko_module=$(find / ! -path "/proc/*" ! -path "/usr/lib/modules/*" ! -path "/boot/*" -regextype posix-extended -regex '.*\.ko')
	if [ $ko_module ];then
		echo -e "\e[41;5;30m $ko_module\e[00m"
	else
		echo -e "\e[00;1;32m[PASS] 未找到可疑的.ko模块\e[00m"
	fi
	echo -e "\n"
}

ssh_check() {
	echo -e "\033[44;1;33m SSH检查 \033[0m"
	#SSH 爆破IP
	echo -e "\e[00;1;33m[+] SSH爆破\e[00m"
	echo -e "\e[00;36m[Command]: ag -a 'authentication failure' /var/log/secure* | awk '{print $14}' | awk -F '=' '{print $2}' | ag '\d+\.\d+\.\d+\.\d+' | sort | uniq -c | sort -nr | head -n 25\e[00m"
	echo -e "\e[00;36m[Command]: ag -a 'authentication failure' /var/log/auth.* | awk '{print $14}' | awk -F '=' '{print $2}' | ag '\d+\.\d+\.\d+\.\d+' | sort | uniq -c | sort -nr | head -n 25\e[00m"
	if [ $OS = 'Centos' ]; then
		ag -a 'authentication failure' /var/log/secure* | awk '{print $14}' | awk -F '=' '{print $2}' | ag '\d+\.\d+\.\d+\.\d+' | sort | uniq -c | sort -nr | head -n 25
	else
		ag -a 'authentication failure' /var/log/auth.* | awk '{print $14}' | awk -F '=' '{print $2}' | ag '\d+\.\d+\.\d+\.\d+' | sort | uniq -c | sort -nr | head -n 25
	fi
	echo -e "\n"

	#SSHD
	echo -e "\e[00;1;33m[+] SSHD \e[00m"
	echo -e "\e[00;36m[Command]: stat /usr/sbin/sshd\e[00m"
	stat /usr/sbin/sshd
	echo -e "\n"

	#SSH 后门配置检查
	echo -e "\e[00;1;33m[+] SSH 后门配置 \e[00m"
	echo -e "\e[00;36m[Command]: grep LocalCommand <~/.ssh/config\e[00m"
	echo -e "\e[00;36m[Command]: grep ProxyCommand <~/.ssh/config\e[00m"
	if [ -e "$HOME/.ssh/config" ]; then
		grep LocalCommand <~/.ssh/config
		grep ProxyCommand <~/.ssh/config
	else
		echo -e "\e[00;1;32m[PASS] 未发现SSH 后门配置文件\e[00m"
	fi
	echo -e "\n"

	#SSH inetd后门检查
	echo -e "\e[00;1;33m[+] SSH inetd后门检查 \e[00m"
	echo -e "\e[00;36m[Command]: grep -E '(bash -i)' </etc/inetd.conf\e[00m"
	if [ -e "/etc/inetd.conf" ]; then
		grep -E '(bash -i)' </etc/inetd.conf
	else
		echo -e "\e[00;1;32m[PASS] 未发现SSH inetd后门\e[00m"
	fi
	echo -e "\n"

	#SSH key
	echo -e "\e[00;1;33m[+] SSH key\e[00m"
	echo -e "\e[00;36m[Command]: cat /root/.ssh/authorized_keys\e[00m"
	sshkey=${HOME}/.ssh/authorized_keys
	if [ -e "${sshkey}" ]; then
		cat ${sshkey}
	else
		echo -e "\e[00;1;32m[PASS] SSH key文件不存在\e[00m"
	fi
	echo -e "\n"
}

webshell_check() {
	echo -e "\033[44;1;33m Webshell检查 \033[0m"
	#PHP webshell查杀
	echo -e "\e[00;1;33m[+] PHP webshell查杀\e[00m"
	echo -e "\e[00;36m[Command]: \e[00m"
	ag --php -l -s -i 'array_map\(|pcntl_exec\(|proc_open\(|popen\(|assert\(|phpspy|c99sh|milw0rm|eval?\(|\(gunerpress|\(base64_decoolcode|spider_bc|shell_exec\(|passthru\(|base64_decode\s?\(|gzuncompress\s?\(|gzinflate|\(\$\$\w+|call_user_func\(|call_user_func_array\(|preg_replace_callback\(|preg_replace\(|register_shutdown_function\(|register_tick_function\(|mb_ereg_replace_callback\(|filter_var\(|ob_start\(|usort\(|uksort\(|uasort\(|GzinFlate\s?\(|\$\w+\(\d+\)\.\$\w+\(\d+\)\.|\$\w+=str_replace\(|eval\/\*.*\*\/\(' $webpath
	ag --php -l -s -i '^(\xff\xd8|\x89\x50|GIF89a|GIF87a|BM|\x00\x00\x01\x00\x01)[\s\S]*<\?\s*php' $webpath
	ag --php -l -s -i '\b(assert|eval|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec)\b[\/*\s]*\(+[\/*\s]*((\$_(GET|POST|REQUEST|COOKIE)\[.{0,25})|(base64_decode|gzinflate|gzuncompress|gzdecode|str_rot13)[\s\(]*(\$_(GET|POST|REQUEST|COOKIE)\[.{0,25}))' $webpath
	ag --php -l -s -i '\$\s*(\w+)\s*=[\s\(\{]*(\$_(GET|POST|REQUEST|COOKIE)\[.{0,25});[\s\S]{0,200}\b(assert|eval|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec)\b[\/*\s]*\(+[\s"\/*]*(\$\s*\1|((base64_decode|gzinflate|gzuncompress|gzdecode|str_rot13)[\s\("]*\$\s*\1))' $webpath
	ag --php -l -s -i '\b(filter_var|filter_var_array)\b\s*\(.*FILTER_CALLBACK[^;]*((\$_(GET|POST|REQUEST|COOKIE|SERVER)\[.{0,25})|(eval|assert|ass\\x65rt|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec))' $webpath
	ag --php -l -s -i "\b(assert|eval|system|exec|shell_exec|passthru|popen|proc_open|pcntl_exec|include)\b\s*\(\s*(file_get_contents\s*\(\s*)?[\'\"]php:\/\/input" $webpath
	echo -e "\n"
	
	#JSP webshell查杀
	echo -e "\e[00;1;33m[+] JSP webshell查杀\e[00m"
	ag --jsp -l -s -i '<%@\spage\simport=[\s\S]*\\u00\d+\\u00\d+|<%@\spage\simport=[\s\S]*Runtime.getRuntime\(\).exec\(request.getParameter\(|Runtime.getRuntime\(\)' $webpath
	echo -e "\n"
}

poison_check() {
	echo -e "\033[44;1;33m 供应链投毒检测 \033[0m"
	#Python2 pip 检测
	echo -e "\e[00;1;33m[+] Python2 pip 检测\e[00m"
	echo -e "\e[00;36m[Command]: pip freeze | ag 'istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request='\e[00m"
	pip freeze | ag "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request="
	echo -e "\n"

	#Python3 pip 检测
	echo -e "\e[00;1;33m[+] Python3 pip 检测\e[00m"
	echo -e "\e[00;36m[Command]: pip3 freeze | ag 'istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request='\e[00m"
	pip3 freeze | ag "istrib|djanga|easyinstall|junkeldat|libpeshka|mumpy|mybiubiubiu|nmap-python|openvc|python-ftp|pythonkafka|python-mongo|python-mysql|python-mysqldb|python-openssl|python-sqlite|virtualnv|mateplotlib|request="
	echo -e "\n"
}

miner_check() {
	echo -e "\033[44;1;33m 挖矿木马检查 \033[0m"
	#常规挖矿进程检测
	echo -e "\e[00;1;33m[+] 常规挖矿进程检测\e[00m"
	normal_miner_1=$(ps aux | ag "systemctI|kworkerds|init10.cfg|wl.conf|crond64|watchbog|sustse|donate|proxkekman|test.conf|/var/tmp/apple|/var/tmp/big|/var/tmp/small|/var/tmp/cat|/var/tmp/dog|/var/tmp/mysql|/var/tmp/sishen|ubyx|cpu.c|tes.conf|psping|/var/tmp/java-c|pscf|cryptonight|sustes|xmrig|xmr-stak|suppoie|ririg|/var/tmp/ntpd|/var/tmp/ntp|/var/tmp/qq|/tmp/qq|/var/tmp/aa|gg1.conf|hh1.conf|apaqi|dajiba|/var/tmp/look|/var/tmp/nginx|dd1.conf|kkk1.conf|ttt1.conf|ooo1.conf|ppp1.conf|lll1.conf|yyy1.conf|1111.conf|2221.conf|dk1.conf|kd1.conf|mao1.conf|YB1.conf|2Ri1.conf|3Gu1.conf|crant|nicehash|linuxs|linuxl|Linux|crawler.weibo|stratum|gpg-daemon|jobs.flu.cc|cranberry|start.sh|watch.sh|krun.sh|killTop.sh|cpuminer|/60009|ssh_deny.sh|clean.sh|\./over|mrx1|redisscan|ebscan|barad_agent|\.sr0|clay|udevs|\.sshd|/tmp/init|xmr|xig|ddgs|minerd|hashvault|geqn|\.kthreadd|httpdz|pastebin.com|sobot.com|kerbero|2t3ik|ddgs|qW3xt|ztctb" | ag -v 'ag')
	normal_miner_2=$(find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/run/*" ! -path "/boot/*" -regextype posix-extended -regex '.*systemctI|.*kworkerds|.*init10.cfg|.*wl.conf|.*crond64|.*watchbog|.*sustse|.*donate|.*proxkekman|.*cryptonight|.*sustes|.*xmrig|.*xmr-stak|.*suppoie|.*ririg|gg1.conf|.*cpuminer|.*xmr|.*xig|.*ddgs|.*minerd|.*hashvault|\.kthreadd|.*httpdz|.*kerbero|.*2t3ik|.*qW3xt|.*ztctb|.*miner.sh' -type f)
	if [ "$normal_miner_1" -o "$normal_miner_2" ];then
		echo -e "$normal_miner_1 \n$normal_miner_2"
	else
		echo -e "\e[00;1;32m[PASS] 未检测到常规挖矿进程\e[00m"
	fi
	echo -e "\n"

	#Ntpclient 挖矿木马检测
	echo -e "\e[00;1;33m[+] Ntpclient 挖矿木马检测\e[00m"
	ntpclient_miner_1=$(find / ! -path "/proc/*" ! -path "/sys/*" ! -path "/boot/*" -regextype posix-extended -regex 'ntpclient|Mozz')
	ntpclient_miner_1=$(ls -alh /tmp/.a /var/tmp/.a /run/shm/a /dev/.a /dev/shm/.a 2>/dev/null)
	if [ "$ntpclient_miner_1" -o "$ntpclient_miner_2" ];then
		echo -e "$ntpclient_miner_1 \n$ntpclient_miner_2"
	else
		echo -e "\e[00;1;32m[PASS] 未检测到Ntpclient 挖矿木马\e[00m"
	fi
	echo -e "\n"

	#WorkMiner 挖矿木马检测
	echo -e "\e[00;1;33m[+] WorkMiner 挖矿木马检测\e[00m"
	work_miner_1=$(ps aux | ag "work32|work64|/tmp/secure.sh|/tmp/auth.sh" | ag -v 'ag')
	work_miner_2=$(ls -alh /tmp/xmr /tmp/config.json /tmp/secure.sh /tmp/auth.sh /usr/.work/work64 2>/dev/null)
	if [ "$work_miner_1" -o "$work_miner_2" ];then
		echo -e "$work_miner_1 \n$work_miner_2"
	else
		echo -e "\e[00;1;32m[PASS] 未检测到WorkMiner 挖矿木马\e[00m"
	fi
	echo -e "\n"
}

rkhunter_install() {
	echo -e "\033[44;1;33m Rkhunter安装 \033[0m"
	#Rkhunter查杀
	echo -e "\e[00;1;33m[+] Rkhunter查杀\e[00m"
	echo -e "\e[00;36m[Command]: rkhunter --checkall --sk | ag -v 'OK|Not found|None found'\e[00m"
	if rkhunter >/dev/null 2>&1; then
		rkhunter --checkall --sk | ag -v 'OK|Not found|None found'
	else
		if [ -e "/tmp/rkhunter.tar.gz" ]; then
			cd /tmp && tar -zxvf /tmp/rkhunter.tar.gz >/dev/null 2>&1
			cd /tmp/rkhunter-1.4.6/ && ./installer.sh --install >/dev/null 2>&1
			rkhunter --checkall --sk | ag -v 'OK|Not found|None found'
		else
			echo -e "未找到 rkhunter.tar.gz 软件包，正在尝试下载..."
			curl --connect-timeout 5 -o /tmp/rkhunter.tar.gz https://nchc.dl.sourceforge.net/project/rkhunter/rkhunter/1.4.6/rkhunter-1.4.6.tar.gz >/dev/null
			tar -zxvf /tmp/rkhunter.tar.gz >/dev/null 2>&1
			cd /tmp/rkhunter-1.4.6/ && ./installer.sh --install >/dev/null 2>&1
			rkhunter --checkall --sk | ag -v 'OK|Not found|None found'
		fi
	fi
}

risk_check() {
	echo -e "\033[44;1;33m 风险&漏洞检查 \033[0m"
	#Redis弱密码检测
	echo -e "\e[00;1;33m[+] Redis弱密码检测\e[00m"
	echo -e "\e[00;36m[Command]: cat /etc/redis/redis.conf 2>/dev/null | ag '(?<=requirepass )(test|123456|admin|root|12345678|111111|p@ssw0rd|test|qwerty|zxcvbnm|123123|12344321|123qwe|password|1qaz|000000|666666|888888)'\e[00m"
	redis_weak_password=$(cat /etc/redis/redis.conf 2>/dev/null | ag '(?<=requirepass )(test|123456|admin|root|12345678|111111|p@ssw0rd|test|qwerty|zxcvbnm|123123|12344321|123qwe|password|1qaz|000000|666666|888888)')
	if [ "$redis_weak_password" ];then
		echo $redis_weak_password
	else
		echo -e "\e[00;1;32m[PASS] 未检测到Redis弱密码\e[00m"
	fi
	echo -e "\n"
}

prerequisites_setting
base_check
network_check
crontab_check
env_check
user_check
service_check
bash_check
file_check
rootkit_check
ssh_check
webshell_check
poison_check
miner_check
rkhunter_install
risk_check